...
from mantis.fundamental.utils.timeutils import TimedTask
...
class BaseService(ServiceBase):
    ...
    def init(self,**kwargs):
        ...
        self.service_id = self.cfgs.get('service_id')
        self.service_type = self.cfgs.get('service_type'
        )
    ...
    def initCommandChannels(self):
        ...
        if 'get' in channels:
            ...
            addr = 'redis/{service_type}.{service_id}/queue'.format(service_type=self.service_type,service_id = self.service_id)
        ...
        if 'sub' in channels:
            ...
            addr = 'redis/{service_type}.{service_id}/pubsub'.format(service_type=self.service_type, service_id=self.service_id)
    ...
    def startTimer(self,back,user=None,timeout = 1):
        ...
        self.timed_taskes.append(task)
        task.start()


    def removeTimer(self,task):
        task.stop()
        self.timed_taskes.remove(task)
