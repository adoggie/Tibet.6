...
def prodinfo():
    ...
    if Strategy.Product.Stk_UseableAmt > 0:#仓位定义：可用资金*10%，固定舍零股处理
        Stk_OrderQty = Stk_OrderQty = int((Strategy.Product.Stk_UseableAmt * 0.1) / stk.KnockPrice) - int((Strategy.Product.Stk_UseableAmt * 0.1) / stk.KnockPrice % 100)
...
def Order(stk):
    ...
    try:
        while ThreadFlag:
            ...
            print u'当前涨跌幅stk.DiffRate：%s' % stk.DiffRate
            if stk.DiffRate > 0.01 and Stk_pos >= 0 and S_flag == 0:
                print 'Order_S'
                      
                order_id = Strategy.Order(OrderItem(stk.ServerCode, 'S', Stk_OrderQty, stk.KnockPrice))
                OrigSerailNo_S.append(order_id)

                B_flag,S_flag  = 1
        
            elif stk.DiffRate < -0.01 and Strategy.Product.Stk_UseableAmt >0 and B_flag == 0:
                print 'Order_B'
                order_id = Strategy.Order(OrderItem(stk.ServerCode, 'B', Stk_OrderQty, stk.KnockPrice))
                OrigSerailNo_B.append(order_id)
...
def RtsChanged(items):#实时委托回报监控
    ...
    for rts in items:
        if rts.Type==0:
            print '收到委托确认：委托号=%s , 代码=%s,委托数量=%i, 委托价格=%i, 状态=%s' % (rts.OrigSerialNo, rts.ServerCode, rts.OrderQty, rts.OrderPrice,rts.StatusCode)     
        else:
            print '收到成交信息：委托号=%s , 代码=%s,成交数量=%i, 成交价格=%i' % (rts.OrigSerialNo, rts.ServerCode, rts.KnockQty, rts.KnockAmt)  
...
def start(market,strategy,orderitem):
    global Market, Strategy, OrderItem,stk
    Market = market
    Strategy = strategy
    OrderItem = orderitem
    ...
    t=threading.Thread(target=Order(stk))
    t.start()
